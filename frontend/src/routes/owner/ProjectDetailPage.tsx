...
function TabButton({ label, icon: IconC, isActive, onClick }: { label: string; icon: (p: IP) => JSX.Element; isActive: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      ...
      border: "none", borderBottom: `3px solid ${isActive ? "#3b82f6" : "transparent"}`, background: isActive ? "rgba(59,130,246,0.05)" : "transparent",
      color: isActive ? "#1e293b" : "#94a3b8", cursor: "pointer", transition: "all .15s",
    }}
    onMouseEnter={(e) => { if (!isActive) e.currentTarget.style.color = "#64748b"; }}
    onMouseLeave={(e) => { if (!isActive) e.currentTarget.style.color = "#94a3b8"; }}>
      <IconC size={14} color={isActive ? "#3b82f6" : "#94a3b8"} />{label}
    </button>
  );
}
...
function CardView({ onOpenDrawer, d }: { onOpenDrawer: (wg: UIWorkgroup) => void; d: UIDashboard }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
      {/* Work Sites subtitle + Site Tabs — only shown when multiple sites */}
      {d.worksites.length > 1 && <>
      ...
      {ws && <div key={activeSite} style={{ animation: "fu .3s cubic-bezier(.22,1,.36,1) both" }}>
        <div style={{ marginBottom: 12, borderRadius: 14, overflow: "hidden", background: sc.gradient }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px" }}>
            ...
            <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
              {[{ l: "Budget", v: fmt(ws.budget), c: "#fff" }, { l: "Paid", v: fmt(wsSpent), c: "#34d399" }, { l: "Invoiced", v: fmt(wsInv), c: "#fbbf24" }, { l: "Remaining", v: fmt(ws.budget - wsSpent - wsInv), c: "#94a3b8" }].map((x) =>
                <div key={x.l} style={{ textAlign: "center" as const }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: x.c }}>{x.v}</div>
                  <div style={{ fontSize: 9, fontWeight: 600, color: "rgba(255,255,255,0.5)", textTransform: "uppercase", letterSpacing: "0.06em" }}>{x.l}</div>
                </div>
              )}
            </div>
          </div>
          <div style={{ padding: "0 16px 8px" }}><BudgetBar spent={wsSpent} invoiced={wsInv} total={ws.budget} h={5} /></div>
        </div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 16 }}>
        {[
          { l: "Total Budget", v: fmtFull(d.totalBudget), c: "#0f172a", bg: "#fff" },
          ...
          { l: "Remaining", v: fmtFull(d.totalBudget - d.totalSpent - d.totalInvoiced), c: "#64748b", bg: "#f8fafc" },
        ].map((s, i) => (
          <div key={s.l} style={{ padding: "14px 16px", borderRadius: 14, background: s.bg, border: "1.5px solid #f1f5f9", animation: `fu .28s ${i * 40}ms both` }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 4 }}>{s.l}</p>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {[{ k: "all", l: "All" }, { k: "submitted", l: "Submitted" }, { k: "approved", l: "Approved" }, { k: "paid", l: "Paid" }, { k: "rejected", l: "Rejected" }].map((f) => (
          <button key={f.k} onClick={() => setFilter(f.k)} style={{
            ...
            border: filter === f.k ? "1.5px solid #3b82f6" : "1.5px solid #e2e8f0",
            background: filter === f.k ? P.active.bg : "transparent", color: filter === f.k ? P.active.fg : "#64748b",
          }}>{f.l}</button>
        ))}
      </div>

      {/* Invoice table */}
      <div style={{ borderRadius: 14, overflow: "hidden", border: "1.5px solid #f1f5f9", background: "#fff", marginBottom: 18 }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ background: "linear-gradient(135deg,#0f172a,#1e293b)" }}>
              {["#", "Site", "Workgroup", "Job", "Contractor", "Amount", "Budget", "Status", "Date"].map((h) => (
                <th key={h} style={{ padding: "10px 12px", textAlign: h === "Amount" || h === "Budget" ? "right" : h === "Status" ? "center" : "left", fontSize: 9, fontWeight: 800, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.1em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={9} style={{ padding: 24, textAlign: "center", color: "#94a3b8", fontSize: 13 }}>No invoices match this filter</td></tr>
            )}
            {filtered.map((inv, i) => { const st = statusBadge(inv.status); return (
              <tr key={inv.id} style={{ borderTop: "1px solid #f8fafc", animation: `fu .22s ${i * 30}ms both`, cursor: "pointer" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "#fafbfc"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = ""; }}>
                <td style={{ padding: "10px 12px", color: P.active.fg, fontWeight: 700 }}>INV-{String(i + 1).padStart(3, "0")}</td>
                <td style={{ padding: "10px 12px", color: "#1e293b" }}>{inv.site}</td>
                <td style={{ padding: "10px 12px", color: "#475569" }}>{inv.workgroup}</td>
                <td style={{ padding: "10px 12px", color: "#1e293b", fontWeight: 600 }}>{inv.jobTitle}</td>
                <td style={{ padding: "10px 12px", color: "#94a3b8" }}>{inv.contractor}</td>
                <td style={{ padding: "10px 12px", textAlign: "right", fontWeight: 900, color: "#0f172a" }}>{fmtFull(inv.amount)}</td>
                <td style={{ padding: "10px 12px", textAlign: "right", color: "#94a3b8" }}>{fmtFull(inv.budget)}</td>
                ...
                <td style={{ padding: "10px 12px", color: "#94a3b8", fontSize: 12 }}>{inv.date}</td>
              </tr>
            ); })}
          </tbody>
        </table>
      ...
      {/* Budget by workgroup */}
      <h3 style={{ fontSize: 13, fontWeight: 800, color: "#475569", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
        <DollarI size={14} color="#64748b" />Budget by Workgroup
      </h3>
              ...
              <div key={wg.id} style={{ animation: `fu .22s ${i * 30 + 60}ms both` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: "#1e293b" }}>{wg.title}</span>
                    <span style={{ fontSize: 12, color: "#94a3b8" }}>— {fmt(wg.budget)} budget</span>
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    {wgS === 0 && wgI === 0 && <span style={{ fontSize: 12, color: "#cbd5e1" }}>No spend</span>}
                  </div>
                </div>
...
export function ProjectDetailPage() {
  ...
  if (loading || !d) { return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: 14, color: "#64748b", fontWeight: 600 }}>Loading project...</p>
      </div>
    </div>
  ); }
  ...
  if (error) { return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ textAlign: "center", padding: 32, background: "#fef2f2", borderRadius: 16, maxWidth: 420 }}>
        <p style={{ fontSize: 18, fontWeight: 800, color: "#dc2626", marginBottom: 8 }}>Failed to load project</p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", fontFamily: "'DM Sans',system-ui,sans-serif", opacity: ready ? 1 : 0, transition: "opacity .3s" }}>
      ...
      <div style={{ padding: "9px 16px", background: "linear-gradient(135deg,#0f172a,#1e293b)", flexShrink: 0 }}>
        {/* Breadcrumb */}
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
          <span onClick={() => navigate("/dashboard")} style={{ fontSize: 12, color: "#64748b", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}
            onMouseEnter={(e) => { e.currentTarget.style.color = "#94a3b8"; }}
            onMouseLeave={(e) => { e.currentTarget.style.color = "#64748b"; }}>
            <ChevLI size={12} color="#64748b" />Dashboard
          </span>
          <span style={{ fontSize: 12, color: "#475569" }}>/</span>
          <span style={{ fontSize: 12, color: "#94a3b8" }}>{d.projectTitle}</span>
        </div>
        {/* Title row */}
      ...
      {/* ── Tab Bar ── */}
      <div style={{ display: "flex", borderBottom: "1.5px solid #e2e8f0", background: "#fff", flexShrink: 0, padding: "0 16px" }}>
        ...
        {activeTab === "timeline" && <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, paddingRight: 8 }}>
          {[{ l: "Done", g: P.done.grad }, { l: "Active", g: P.active.grad }, { l: "Queued", g: P.ns.grad }, { l: "Depends", g: "", sym: true }].map((x) =>
            <div key={x.l} style={{ display: "flex", alignItems: "center", gap: 3 }}>{"sym" in x && x.sym ? <ArrI size={10} color="#fbbf24" /> : <div style={{ width: 12, height: 4, borderRadius: 2, background: x.g }} />}<span style={{ fontSize: 9, color: "#94a3b8" }}>{x.l}</span></div>
          )}
        </div>}
      </div>
