...
function GanttView({ d }: { d: UIDashboard }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
      <div style={{ flexShrink: 0, display: "flex", borderBottom: "1.5px solid #ECEAE6" }}>
        <div style={{ width: 200, flexShrink: 0, padding: "7px 12px", background: "linear-gradient(135deg,#1A1814,#3D3529)" }}><span style={{ fontSize: 9, fontWeight: 800, color: "#9C8E7C", textTransform: "uppercase", letterSpacing: "0.12em" }}>Site / Trade / Job</span></div>
        <div style={{ flex: 1, display: "flex", position: "relative", background: "#fafbfc" }}>
          {mos.map((m, i) => <div key={m} style={{ flex: 1, textAlign: "center" as const, padding: "7px 0", borderLeft: i ? "1px solid #ECEAE6" : "none" }}><span style={{ fontSize: 9, fontWeight: 800, color: "#8C7E6A", textTransform: "uppercase", letterSpacing: "0.12em" }}>{m} 2026</span></div>)}
          <div style={{ position: "absolute", top: 0, bottom: 0, width: 2, zIndex: 20, left: `${d2p("2026-03-10")}%`, background: "linear-gradient(180deg,#D44A2E,#E8705A)", animation: "glow 3s ease-in-out infinite" }}>
            <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", fontSize: 7, fontWeight: 900, color: "#fff", background: "#D44A2E", padding: "2px 8px", borderRadius: "0 0 5px 5px", letterSpacing: "0.06em" }}>TODAY</div>
          </div>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: "auto" }}>
        {d.worksites.map((ws, wi) => { const sc = SC[wi % SC.length]; return (
          <div key={ws.name} style={{ animation: `fu .32s ${wi * 70}ms both` }}>
            <div style={{ display: "flex", alignItems: "center", borderBottom: "1.5px solid #ECEAE6", position: "sticky", top: 0, zIndex: 10 }}>
            ...
            {ws.workgroups.map((wg, wgi) => {
              ...
              return (
                <div key={wg.id} style={{ animation: `si .3s ${wi * 70 + wgi * 45 + 60}ms both` }}>
                  <div onClick={() => setExp(isE ? null : wg.id)} style={{ display: "flex", alignItems: "center", borderBottom: `1px solid ${isE ? sc.ring : "#F0EDE8"}`, cursor: "pointer", background: isE ? sc.bg : "transparent" }}>
                    <div style={{ width: 200, flexShrink: 0, padding: "7px 12px", display: "flex", alignItems: "center", gap: 7 }}>
                      ...
                      <div><div style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ fontSize: 11, fontWeight: 700, color: "#1A1814" }}>{wg.title}</span><Badge status={wg.status} /></div>
                        <p style={{ fontSize: 9, color: "#9C8E7C" }}>{wg.contractor} · {dn}/{wg.jobs.length}</p></div>
                    </div>
                    <div style={{ flex: 1, position: "relative", height: 36, display: "flex", alignItems: "center" }}>
                      ...
                      {wg.dependsOn && <div style={{ position: "absolute", top: "50%", transform: "translateY(-50%)", zIndex: 10, left: `${Math.max(s - 2, 0)}%` }}><ArrI size={11} color="#E5A63B" /></div>}
                      <div style={{ position: "absolute", height: 22, borderRadius: 7, overflow: "hidden", left: `${s}%`, width: `${w}%`, background: wg.dependsOn ? "transparent" : "rgba(226,232,240,0.35)", border: wg.dependsOn ? "2px dashed #E5A63B" : `1.5px solid ${sm.p.ring}` }}>
                      ...
                      <div key={job.id} style={{ display: "flex", alignItems: "center", borderBottom: "1px solid rgba(226,232,240,0.3)", animation: `fu .28s ${ji * 35}ms both`, background: sc.bg + "50" }}>
                        <div style={{ width: 200, flexShrink: 0, paddingLeft: 50, paddingRight: 8, paddingTop: 5, paddingBottom: 5, display: "flex", alignItems: "center", gap: 6 }}>
                          {job.status === "complete" ? <div style={{ width: 14, height: 14, borderRadius: 7, background: P.done.grad, display: "flex", alignItems: "center", justifyContent: "center" }}><CheckI size={7} color="#fff" sw={3} /></div>
                            ...
                            : <div style={{ width: 14, height: 14, borderRadius: 7, border: "1.5px solid #C4B5A2", background: "#fff" }} />}
                          <span style={{ fontSize: 10, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: job.status === "complete" ? "#9C8E7C" : "#5C5043", fontWeight: job.status === "complete" ? 500 : 600, textDecoration: job.status === "complete" ? "line-through" : "none" }}>{job.title}</span>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #ECEAE6", background: "#fff", flexShrink: 0, padding: "0 16px" }}>
        ...
        {activeTab === "timeline" && <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10, paddingRight: 8 }}>
          {[{ l: "Done", g: P.done.grad }, { l: "Active", g: P.active.grad }, { l: "Queued", g: P.ns.grad }, { l: "Depends", g: "", sym: true }].map((x) =>
            <div key={x.l} style={{ display: "flex", alignItems: "center", gap: 3 }}>{"sym" in x && x.sym ? <ArrI size={10} color="#E5A63B" /> : <div style={{ width: 12, height: 4, borderRadius: 2, background: x.g }} />}<span style={{ fontSize: 9, color: "#9C8E7C" }}>{x.l}</span></div>
          )}
        </div>}
      </div>
